...
Detector::Detector() {
    ...
    // PCA
    cloud_temp_2d.reset(new pcl::PointCloud<pcl::PointXY>);
    cloud_temp_2d_.reset(new pcl::PointCloud<pcl::PointXY>);
    mean = Eigen::Vector2f::Zero();
    covariance = Eigen::Matrix2f::Zero();
    eigen_vectors = Eigen::Matrix2f::Zero();
    eigen_values = Eigen::Vector2f::Zero();

    // Bounding box
    p1 = Eigen::Vector2f::Zero();
    p2 = Eigen::Vector2f::Zero();
    p3 = Eigen::Vector2f::Zero();
    p4 = Eigen::Vector2f::Zero();
    box_x.reserve(5);
    box_y.reserve(5);
...
void Detector::create_cluster(int cluster_id, std::vector<int>& neighbor_vec) {
    ...
    if (feature_extractor.getOBB(min_point, max_point, position, rotation_matrix)) {
        cluster_vec.emplace_back(cluster_id, indices_set, min_point, max_point, position, rotation_matrix);
    }
    else {
        ROS_INFO("NO FEATURE");
    }
...
void Detector::get_mean() {
    
    for (const auto& elem : cloud_temp_2d->points) {
        mean += Eigen::Vector2f(elem.x, elem.y);
    }
    mean /= cloud_temp->points.size();
}

void Detector::get_covariance() {

    for (const auto& elem : cloud_temp_2d->points) {
        Eigen::Vector2f centered = Eigen::Vector2f(elem.x, elem.y) - mean;
        covariance += centered * centered.transpose();
    }
    covariance /= cloud_temp->points.size();
}
...
void Detector::bounding_box() {
    ...
    extractor_2d.setInputCloud(cloud_2d);
    extractor_2d.setNegative(false);
    ...
    for (size_t i = 0; i < cluster_vec.size(); i++) {
        box_x = {cluster_vec[i].min_point.x, cluster_vec[i].max_point.x, cluster_vec[i].max_point.x, cluster_vec[i].min_point.x};
        box_y = {cluster_vec[i].min_point.y, cluster_vec[i].min_point.y, cluster_vec[i].max_point.y, cluster_vec[i].min_point.y};
